...
import { homeQuadrant, QUADRANT_CENTER_ANGLE, SCENE_FACETS } from "./facets";
import type { SceneFacet } from "./facets";
...
function QuestionPlate({
  ...
  return (
    <Plate position={position} className="questionPlate" z={75}>
...
export function SeedFlow({
  ...
  if (expanded) {
    ...
    return (
      <Plate position={[0, 0.7, 1.3]} className="seedPickPlate" z={75}>
                ...
                <div key={bucket}>
                  ...
                  {members.map((pageType) => (
                    <button
                      ...
                      onMouseEnter={() => onPreviewQuadrant?.(homeQuadrant(pageType, overrides))}
  ...
  // Curated picker: the scope's catalog as cards over the world (≤7 by
  // construction — the root's catalog is 7). Everything else waits behind
  // "more types…". Hovering a card lights its home quadrant.
  const spread = palette.primary.length;
  const step = spread > 1 ? 6.4 / (spread - 1) : 0;
  return (
    <group>
      <Plate position={[0, 1.9, 0.6]} className="seedTitleWrap" z={72}>
      ...
      {palette.primary.map((pageType, index) => (
        <Plate
          key={pageType}
          position={[-3.2 + index * step, index % 2 === 0 ? 0.55 : -0.15, 1.0]}
          className="spatialCardWrap"
          distanceFactor={8}
        >
          <button
            ...
            onClick={() => pick(pageType)}
            onMouseEnter={() => onPreviewQuadrant?.(homeQuadrant(pageType, overrides))}
            onMouseLeave={() => onPreviewQuadrant?.(null)}
            title={typeDescription(pageType)}
            type="button"
          >
      ...
      {palette.rest.length > 0 && (
        <Plate position={[0, -1.15, 1.5]} className="spatialCardWrap" distanceFactor={8}>
...
export function WorldPlate({ node, onOpen, onTrails, onPacket, onClose }: WorldPlateSpec) {
  // Anchor at the node's TOP POLE; the wrapper CSS raises the plate by its
  // own height in screen space, so the arrow tip touches the node always.
  ...
  return (
    <Plate
      ...
      z={60}
    >
      <div className="worldPlate anchoredAbove" role="dialog" aria-label={node.title}>
