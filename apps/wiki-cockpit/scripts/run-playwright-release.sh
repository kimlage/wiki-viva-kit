...
if [ "${1:-}" != "--scope" ] || [ "${2:-}" != "public_required" ] || [ "$#" -ne 2 ]; then
  echo "isolated release launcher supports only --scope public_required" >&2
  exit 1
fi

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
NODE_BIN=$(command -v node) || {
  echo "release runner launcher could not resolve node" >&2
  exit 1
}
while [ -L "$NODE_BIN" ]; do
...
case "$NODE_MAGIC" in
  ...
  *)
    echo "release runner launcher requires a native Node executable" >&2
    exit 1
    ;;
esac
...
exec /usr/bin/env -i \
  ...
  "$NODE_BIN" "$SCRIPT_DIR/run-playwright-release.mjs" "$@"
