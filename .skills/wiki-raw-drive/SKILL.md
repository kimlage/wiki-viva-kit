...
(the directory declared by `paths.raw_root`, which is gitignored). This separation follows the
...
`_drive_manifest.json` under configured `paths.raw_root` (this JSON is
...
2. Download the content with the Drive connector (`download_file_content` /
   `read_file_content`) into configured `paths.raw_root` (the raw cache,
   gitignored).
...
- Never commit raw files; only `_drive_manifest.json` under the configured raw
  root is versioned.
