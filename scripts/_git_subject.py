...
This module intentionally does not import :mod:`wiki_core`.  The cockpit's
Node-only CI job calls the companion ``wiki_git_subject.py`` command before it
creates browser evidence, and that job must not silently depend on PyYAML or
the rest of the Python runtime.
