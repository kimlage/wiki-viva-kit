...
def _verify_source(source_root: Path, source_sha: str) -> Path:
    ...
    top = _run_git(
        source, ["rev-parse", "--show-toplevel"], label="source root"
    ...
    head = _run_git(source, ["rev-parse", "HEAD"], label="source HEAD").decode(
        "ascii", "strict"
    ).strip()
...
def _package_contract(
    ...
    if (
        ...
        or any(not isinstance(profile, str) or ID_RE.fullmatch(profile) is None for profile in profiles)
    ):
        raise VisualEvidenceError(
...
def _public_demo_route(value: object, *, label: str = "route") -> str:
    ...
    for _ in range(MAX_PERCENT_DECODE_ROUNDS):
        ...
        try:
            ...
            raise VisualEvidenceError(f"{label} contains invalid percent encoding") from exc
        if decoded == current:
            break
...
def _output_path(
    raw: Path, *, source_root: Path, must_exist: bool
) -> Path:
    expanded = raw.expanduser()
    if any(part in {"", ".", ".."} for part in expanded.parts if part != expanded.anchor):
    ...
    if not must_exist and exists:
        raise VisualEvidenceError("visual output already exists; evidence is create-once")
...
def _probe_browser_toolchain(source_root: Path) -> dict[str, str]:
    module = _node_playwright_module(source_root)
    try:
        result = subprocess.run(
            ["node", "-e", _BROWSER_PROBE, str(module)],
            cwd=source_root,
            check=False,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
    ...
    try:
        ...
        raise VisualEvidenceError("Playwright Chromium probe was not canonical") from exc
    if (
...
def _write_bundle(
    *,
    ...
    if set(by_profile) != set(profiles) or len(by_profile) != len(observations):
        raise VisualEvidenceError("Chromium did not return one result per visual profile")
...
def _verify_record(
    *,
    ...
    try:
        ...
        raise VisualEvidenceError("visual image failed strict PNG verification") from exc
    if (
        actual_image != image
        ...
        raise VisualEvidenceError(
            "visual image must equal its declared DPR-1 viewport"
        )
    ...
    for current, directories, files in os.walk(visual_root, followlinks=False):
        ...
        for name in [*directories, *files]:
            ...
            if candidate.is_symlink():
                raise VisualEvidenceError("visual evidence inventory contains a symlink")
        for name in files:
            candidate = current_path / name
...
def verify_visual_evidence(
    *,
    ...
    manifest_relative = _canonical_relative_path(
        manifest_ref, label="visual manifest"
    )
    ...
    if (
        ...
        raise VisualEvidenceError("visual manifest fields or canonical bytes are invalid")
    ...
    for index, entry in enumerate(entries):
        if not isinstance(entry, dict) or set(entry) != _MANIFEST_ENTRY_FIELDS:
            raise VisualEvidenceError(f"visual manifest entry {index} fields are invalid")
...
def _build(source_root: Path) -> None:
    try:
        result = subprocess.run(
...
def _start_preview(source_root: Path, port: int) -> subprocess.Popen[bytes]:
    try:
        ...
        raise VisualEvidenceError("exact-source preview could not start") from exc
...
def _capture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    preview = _start_preview(source, port)
    created = False
    try:
