...
COCKPIT_STRINGS: dict[str, dict[str, str]] = {
    "pt": {
        ...
        "proposal_state": "- Estado no momento da compilacao: branch de proposta `wiki/*`; depois do merge, trate esta linha como proveniencia historica e verifique o estado atual do repo/PR antes de agir.",
        "approved_state": "- Estado no momento da compilacao: `main` aprovado; criar uma branch `wiki/*` antes de propor nova mudanca.",
        "branch_state": "- Estado no momento da compilacao: branch `{branch}`; verificar politica de PR antes de publicar ou concluir.",
    ...
    "en": {
        ...
        "proposal_state": "- Compile-time state: proposal branch `wiki/*`; after merge, treat this line as historical provenance and check the current repo/PR state before acting.",
        "approved_state": "- Compile-time state: approved `main`; create a `wiki/*` branch before proposing a new change.",
        "branch_state": "- Compile-time state: branch `{branch}`; check PR policy before publishing or concluding.",
...
def _branch_state_line(branch: str, s: dict[str, str]) -> str:
    """Human-facing compile-time provenance hint for the current branch."""
    if branch == "main":
        return s["approved_state"]
    if branch.startswith("wiki/"):
        return s["proposal_state"]
    return s["branch_state"].format(branch=branch)
