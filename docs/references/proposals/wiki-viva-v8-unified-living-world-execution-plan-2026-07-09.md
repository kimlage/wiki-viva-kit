...
milestone; `f7f95119` keeps the search query, selected page, reader and
closed-dock state in one atomic route transaction across the WebKit/Linux
debounce boundary; `877b586b` is the settled desktop group-geometry milestone;
and the current functional payload is `dbd158a4`, which scopes authored node
offsets to the 3D projection, keeps fallback collection cards reachable and
tests the one-way canvas-to-performance-map transition without accepting a
runtime remount.
...
| Phase 7 - tests/gates | `done` | Python gates, frontend tests, Playwright E2E and diff checks. | Python: 706 passed, 4 skipped. Frontend: 395/395 across 51 files on Node 26 and Node 22.23.1; Node gates 15/15; architecture 0 violations/0 debt; snapshot API, bundle, demo/snapshot drift, OKF and diff checks green. The final browser matrix passed 57 scenarios with 2 real-endpoint opt-in skips across 59 scenarios in dedicated Chromium performance, Chromium desktop, WebKit mobile, forced fallback and Firefox projects; its group-geometry contract covers both public worlds at `390x664`, `390x844` and `1280x900`. Audit reports 0 errors and 3 explicit stale-page warnings. Bundle evidence is 139.11 kB initial JS gzip, 1.73 kB initial CSS gzip and 53.89 kB largest lazy chunk. Deterministic operation/input/source compilations match HEAD. Whole-tree Ruff is not a release gate and retains pre-existing bootstrap/style debt. |
| Phase 8 - visual validation | `done` | QA evidence package for desktop, mobile, fallback and private read-only pass. | The exact Alex route was traversed in the in-app Browser at `1280x900`, `390x664` and `390x844`: Q1-Q4, every visible real page and semantic group, collection examples and Sources -> Evidence origins. Every node-like target is a native mouse/keyboard/focus control; groups expose count, description and examples; a member reaches a reader or real center in at most two steps; recenter resets the lens to `all`; breadcrumb and center match the destination; no group loops; and one canvas survives the complete route. Both instructional and dense fixtures now also prove settled pairwise group geometry, intended `elementFromPoint` ownership, in-viewport bounds and zero document scroll at desktop. The mission foreground owns mobile without the compass competing, while the closed mission and compass keep Q2 targets pointer-safe. The strict Chromium performance project passed both normal and dense budgets at p95 <= 33.33 ms. Reader/Guide/dock foreground arbitration, reduced motion, WebKit touch and forced fallback remain green, with no current application console/network failure observed. |
| Phase 9 - downstream repository upgrades | `done` | Consumer inventory, upgrade package, migration reports, pilot/wave status and redacted QA evidence. | The portable implementation source is pinned to payload `dbd158a4`, with exact-tree comparison, inventory, allow/block lists, runtime-config protection, report compiler/schema, rollback and compatibility window. The private pilot completed the controlled real-data snapshot/action/collection refresh and the two-file fallback reachability sync under draft PR #208; its clean 57-pass/2-skip local browser matrix retains `b942735f`, `1d801f1c`, `a483ad02`, `d4a3c890`, `f7f95119` and `877b586b` as historical milestones. No private data was imported upstream; both human merge gates remain external. |
| Phase 10 - documentation/release | `done` | README, cockpit README, modular-blocks, extending-the-kit, examples, diagrams and release notes. | Runtime/upgrade guides, README surfaces, extension/block guidance, command reference, release candidate, implementation payload `dbd158a4` and PR #61 are versioned. Stable release remains the separate human merge/tag gate above. |
...
| UX-R16 - atomic search-reader continuity | `done` | Enter from global search must keep query, page, reader and closed-dock state together after the debounce window, including adaptive WebKit fallback; repeating the same query later must still work. | Remote WebKit/Linux evidence showed a late 250 ms query write replaying the pre-close Create route over the requested reader. Payload `f7f95119` cancels an existing timer, marks only an uncommitted direct submit, keeps that marker for the lifetime of the exact submitted draft, releases it on a genuine edit and never creates it for an already-committed query. The real WebKit regression crosses the debounce twice, asserts the reader remains primary and proves later queries remain usable; it passes five CI-mode repetitions and the complete 57-pass matrix. The browser test is authoritative because the Node 22 Happy DOM/React harness does not dispatch this text-input synthetic keydown, while the same unit fixture does dispatch it on Node 26. |
| UX-R17 - settled desktop semantic-group integrity | `done` | Full explanatory collection handles must remain pairwise disjoint, correctly hit-testable and navigationally progressive after authored motion settles on desktop as well as every reviewed phone height. | The private real-world Browser audit exposed a Q2 collision; the public `dense_stress` fixture then reproduced Q1/Q2 and Q2 family contention, while Alex exposed the Q4 area/content graze. Payload `877b586b` gives repeated semantic families stable desktop lanes without changing their quadrant meaning. The regression waits beyond the longest spatial transition and covers instructional/dense worlds at `390x664`, `390x844` and `1280x900`: 0 overlaps, intended center hit ownership, viewport/document bounds, mouse/keyboard collection routing, truthful breadcrumb/lens and one canvas. All 57 runnable browser scenarios, 395 frontend tests on Node 26 and Node 22, 15 Node gates, architecture, build and bundle checks pass. |
| UX-R18 - adaptive fallback group reachability | `done` | A measured 3D-to-2D performance fallback must preserve the same shell, semantic route and reachable collection cards; 3D-only authored offsets must never move fallback controls outside the viewport. | Private Linux/WebKit evidence exposed `family:event` inheriting a Q2 3D translate after the runtime honestly latched `performance_budget`. Payload `dbd158a4` scopes every phone family offset to `data-world-fallback-active=false`, verifies fallback cards have `translate: none`, scrolls each current fallback target into view, waits for reader/collection exit before the next touch and accepts only the one-way canvas-to-map transition inside the same scene shell. Public and private full local matrices each pass 57 scenarios with 2 expected endpoint skips and no flaky result in the accepted run; the private snapshot still contains 561 real pages. |
