...
Rc24 is the prospective unpinned source transition and grants no adoption or
...
| Prospective public rc24 source transition | `unreleased` / `validation_pending`; predecessor `e97371490091416466825bc6c6d79ed984d480ee`; exact source pin not yet created | Pre-pin worktree: 3 shared CLI fixture consumers, 2 package/inventory contracts, 50 router/runtime controls and 33 focused Chromium/WebKit/Firefox checks pass under Node 22; production build and 102-public-plus-2-downstream matrix contract pass; complete exact matrix remains required and no prior subject result is reusable | Derive the CLI fixture from `VISUAL_PROFILE_CONTRACTS`; require native release specs to assert `data-runtime-mode=v8` and legacy compatibility specs to assert `compat` | Unpinned local source transition only; no capsule, downstream QA, push, merge, tag or publication authority |
...
| Public demos | Seven executable base scenarios, nine Genesis stages and two pack showcases exist | Historical rc21 demo, pack, Genesis, accessibility, compatibility, failure and source-lifecycle cells pass in its 102-cell receipt | Gallery, source/failure/compatibility/accessibility worlds and pack Chronoscope are concrete | Retain as historical rc21 proof; rc24 must execute and bind its own complete demo matrix before any release claim |
| Visual system | Light/dark themes, three densities, semantic tokens, licensed asset manifest, WebGL and 2D fallback render | Historical rc21 passes 102 cells covering PT-BR, keyboard, touch, mobile, fallback and the RT-151 reading-order regression | The inspected RT-151 overlap is closed with public synthetic geometry; transition frames require settling and VoiceOver remains human | Historical automation/E3 evidence only; rc24 needs a new exact visual authority plus human conceptual/privacy/VoiceOver review before promotion |
...
shared stale synthetic CLI route helper. All new certification starts from a
new rc24 subject, and public publication remains unauthorized.
...
prospectively and remains unpinned until a later metadata-only boundary. Public
...
| Rc24 | `unreleased` / `validation_pending`, predecessor `e9737149...`; exact source pin still absent | Pre-pin focused controls pass: 3 shared CLI fixtures, 2 package contracts, 50 router/runtime tests, 33 cross-browser route/runtime checks, production build and release-matrix contract. Still requires a fresh metadata pin, complete matrix, productive capture, capsule and independent verifier before Lane B QA |
...
capsule. Rc24 is the next prospective unpublished source transition. A later
...
or import it. Start certification from a fresh rc24 subject that inherits no
prior status, receipt, manifest or digest claim. Rc24 stays local and
