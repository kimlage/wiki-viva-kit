...
rc26 is immutable `historical_certification_failed` evidence. RT-162 is
implemented in exact rc27 source
`ba7ee19457436993edc7ff8a838b34c5b864fd98`, pinned only for validation: replace the unsafe
multiprocessing `fork` fixture with `spawn`, register portable Python as
`python3 -m pytest -q -W error tests/`, expand synthetic host-path negatives and
repeat validation, candidate formation, productive capture and certification
from zero. Public push and publication remain unauthorized.
...
| Exact public rc27 validation source | Exact source `ba7ee19457436993edc7ff8a838b34c5b864fd98`; package `wiki-viva-v8-rc27` remains `validation_pending`; package-file/canonical-package/tree `e092bd63...` / `29225e68...` / `0d31d17f...`, 521 entries | Synthetic correction replaces the multiprocessing `fork` fixture with `spawn`, registers `python3 -m pytest -q -W error tests/` and expands host-path scanner negatives | Every warning must fail before a supposedly successful log can enter public evidence; no post-processing or manual redaction may create authority | Validation-only and non-promotional. Complete validation, separate candidate boundary, new productive capture and Lane A certification remain pending; public push/publication is unauthorized |
...
| Public demos | Seven executable base scenarios, nine Genesis stages and two pack showcases exist | Exact rc26 passed its first/only 102-cell public matrix with 0 skips/retries, including demo, pack, Genesis, accessibility, compatibility, failure and source-lifecycle cells | Gallery, source/failure/compatibility/accessibility worlds and pack Chronoscope are concrete | Rc26 browser/capture proof remains exact-subject historical evidence only; the failed certification grants no capsule or adoption authority, and rc27 must repeat every stage |
| Visual system | Light/dark themes, three densities, semantic tokens, licensed asset manifest, WebGL and 2D fallback render | Exact rc26 passed 102 browser cells and its first productive four-profile capture, manifest `6681e1f751ecd157854a4c3d78360a79f981100a4eda97ec377189ea9566614f` | Native/fallback/profile behavior was captured without granting release authority; transition frames still require settling and VoiceOver remains human | Rc24 and rc26 visual evidence is historical and cannot be reused. Rc27 requires a fresh productive capture, certification and human conceptual/privacy/VoiceOver review |
...
for a separately certified exact rc27-or-later subject. Public closure never
...
| RT-150 / RT-162 | rc26 became `candidate` only after exact validation, then passed productive capture and all six registered commands; the public-evidence scanner still rejected the transaction before attestation | Preserve rc26 as immutable failed-certification evidence. Validate exact rc27 source `ba7ee194...`, then repeat candidate/capture/certification and independently verify its capsule before fresh private preflight or any promotion boundary |
...
All new certification must begin from a separately pinned and fully validated
rc27-or-later source. Public push and publication remain unauthorized.
...
receipt, attestation, trust or Lane B authority exists. Rc27 is pinned only for
exact validation at `ba7ee19457436993edc7ff8a838b34c5b864fd98`. Public push
and publication remain unauthorized.

#### Rc22 failed capture, rc23/rc25 failed validation, rc24/rc26 failed certification and rc27 boundary — 2026-07-14
...
| Rc27 exact validation source | Exact source `ba7ee19457436993edc7ff8a838b34c5b864fd98`; validation-pending package-file/canonical-package/tree `e092bd63...` / `29225e68...` / `0d31d17f...`, 521 entries; no candidate, capture, capsule or certification identity exists | Replaces the unsafe multiprocessing `fork` fixture with `spawn`; registers portable Python as `python3 -m pytest -q -W error tests/`; expands public synthetic host-path negatives; requires a completely fresh validation, candidate boundary, productive capture and certification |

This rc27 correction is pinned for validation only. The already sealed v2 downstream C3 and all of
...
rc27-or-later authority exists. The rehearsal must use a fresh clone and branch
...
command result may be reused. Rc27 is pinned only for exact validation at
`ba7ee19457436993edc7ff8a838b34c5b864fd98`; it uses `spawn`,
warnings-as-errors and expanded host-path scanner controls, but its fresh
validation/candidate/capture/certification sequence remains pending. A later
...
receipt, attestation or trust by hand. Use exact rc27 source
`ba7ee19457436993edc7ff8a838b34c5b864fd98`, pinned only for validation, with
the multiprocessing fixture changed from `fork` to `spawn`, the
registered portable gate set to `python3 -m pytest -q -W error tests/` and
expanded public synthetic host-path negatives. Then run one complete
validation, a separate candidate boundary, one new productive capture, one
certification and independent capsule verification from zero. Keep rc27 local
and non-promotional until downstream QA, remote CI, human review, merge and a
...
rc27 formation. No public push is authorized by this plan update.
