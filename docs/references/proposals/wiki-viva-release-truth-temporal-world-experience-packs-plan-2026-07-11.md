...
defect. Rc27 is immutable `historical_validation_failed` evidence. Rc28 remains
prospective and unpinned; it must repair each lifecycle boundary with public
synthetic fixtures and begin validation from a fresh exact source. Public push
...
| Prospective public rc28 source | Not pinned; no release/source/package/tree identity exists yet | Public synthetic corrections must kill/reap child processes, join workers and close Codex-job pipes; use a `Popen` context plus `communicate()` and explicit stream closure for batch Git; and use `communicate()` for the snapshot reader lease | Focused timeout and lifecycle regressions must pass before a fresh complete validation is pinned | Prospective only. Form a new exact source, then repeat validation, candidate, productive capture and certification from zero; public push/publication remains unauthorized |
...
| Public demos | Seven executable base scenarios, nine Genesis stages and two pack showcases exist | Exact rc26 passed its first/only 102-cell public matrix with 0 skips/retries, including demo, pack, Genesis, accessibility, compatibility, failure and source-lifecycle cells; rc27 stopped in Python before browser | Gallery, source/failure/compatibility/accessibility worlds and pack Chronoscope are concrete | Rc26 browser/capture proof remains exact-subject historical evidence only; rc27 grants no browser or release authority, and rc28 must repeat every stage |
| Visual system | Light/dark themes, three densities, semantic tokens, licensed asset manifest, WebGL and 2D fallback render | Exact rc26 passed 102 browser cells and its first productive four-profile capture, manifest `6681e1f751ecd157854a4c3d78360a79f981100a4eda97ec377189ea9566614f`; rc27 never reached capture | Native/fallback/profile behavior was captured without granting release authority; transition frames still require settling and VoiceOver remains human | Rc24 and rc26 visual evidence is historical and cannot be reused. Rc28 requires a fresh productive capture, certification and human conceptual/privacy/VoiceOver review |
...
for a separately certified exact rc28-or-later subject. Public closure never
...
| RT-150 / RT-162 / RT-163 | rc26 became `candidate` only after exact validation, then passed productive capture and all six registered commands; the public-evidence scanner still rejected the transaction before attestation. Rc27 then stopped in its first `-W error` Python validation with 46 lifecycle failures before browser or candidate formation | Preserve rc26 as immutable failed-certification evidence and rc27 as immutable failed-validation evidence. Form an unpinned rc28 source with the three public synthetic lifecycle corrections, then repeat exact validation, candidate/capture/certification and independently verify its capsule before fresh private preflight or any promotion boundary |
...
begin from a separately pinned and fully validated rc28-or-later source. Public
...
`historical_validation_failed`; rc28 is prospective and unpinned. Public push
and publication remain unauthorized.

#### Rc22 failed capture, rc23/rc25/rc27 failed validation, rc24/rc26 failed certification and rc28 boundary — 2026-07-14
...
| Rc28 prospective correction | No source, validation metadata, package, tree or candidate identity is pinned | Kill/reap subprocesses, join threads and close pipes in Codex jobs plus add a timeout regression; use a `Popen` context, `communicate()` and explicit stream closure for batch Git; use `communicate()` for the reader lease. All fixtures remain public and synthetic | Form a new exact source only after focused lifecycle regressions pass; then execute validation, candidate boundary, new productive capture and certification from zero. No public push/publication authority |

Rc27 is frozen as historical failed-validation evidence and rc28 remains
prospective and unpinned. The already sealed v2 downstream C3 and all of its
...
rc28-or-later authority exists. The rehearsal must use a fresh clone and branch
...
later authority stage were not started. Rc28 is the unpinned prospective
public-synthetic lifecycle correction. A later
...
relabel, promote or import it. Form rc28 from public synthetic lifecycle fixes:
kill/reap/join/close Codex-job resources with a timeout regression, enforce the
`Popen` context plus `communicate()` and stream-close contract for batch Git,
and drain the snapshot reader lease with `communicate()`. Do not pin rc28 until
those focused regressions pass. Then run one complete exact validation, a
separate candidate boundary, one new productive capture, one certification and
independent capsule verification from zero. Keep rc28 local and
...
rc28 formation. No public push is authorized by this plan update.
