...
rc26 is immutable `historical_certification_failed` evidence. RT-162 requires a
fresh prospective rc27 source, still unpinned: replace the unsafe
...
| Prospective public rc27 source | Unpinned; no source, package, candidate, capture or authority identity exists | Planned synthetic correction replaces the multiprocessing `fork` fixture with `spawn`, registers `python3 -m pytest -q -W error tests/` and expands host-path scanner negatives | Every warning must fail before a supposedly successful log can enter public evidence; no post-processing or manual redaction may create authority | Form a fresh source and repeat complete validation, candidate boundary, productive capture and Lane A certification; public push/publication remains unauthorized |
...
| RT-150 / RT-162 | rc26 became `candidate` only after exact validation, then passed productive capture and all six registered commands; the public-evidence scanner still rejected the transaction before attestation | Preserve rc26 as immutable failed-certification evidence. Form and validate a fresh rc27-or-later source, repeat capture/certification and independently verify its capsule before fresh private preflight or any promotion boundary |
...
receipt, attestation, trust or Lane B authority exists. Rc27 remains
prospective and unpinned. Public push and publication remain unauthorized.
...
| Rc27 prospective correction | Source unpinned; no package, candidate, capture or certification identity exists | Replace the unsafe multiprocessing `fork` fixture with `spawn`; register portable Python as `python3 -m pytest -q -W error tests/`; expand public synthetic host-path negatives; require a completely fresh validation, candidate boundary, productive capture and certification |
...
command result may be reused. Rc27 is prospective and unpinned; it must use
`spawn`, warnings-as-errors and expanded host-path scanner controls before a
fresh validation/candidate/capture/certification sequence. A later
...
receipt, attestation or trust by hand. Form a fresh, currently unpinned rc27
source with the multiprocessing fixture changed from `fork` to `spawn`, the
