...
required two writes to reach a fixed point. Rc32 is prospective and unpinned:
fix one-write idempotence with public synthetic data, regenerate the artifact,
then run a wholly new validation/candidate/capture/certification sequence.
...
| Prospective public rc32 source formation | Active package `unreleased@6c8fce74`; no rc32 source pin or reusable validation evidence | Existing-stale, initially-missing, JSON, custom-output and relative-root synthetic fixed-point regressions pass; 198 focused operational/workflow/package/lane tests pass with warnings as errors. One dashboard write is immediately check-clean, SHA-256 `b632bf8e...`; audit/public-audit/methodology/operation/input/semantic/snapshot/packs are green | Preserve RT-164; RT-165 excludes self-derived recent state across Markdown/JSON and rejects target escape; RT-166 aligns local/CI warning/freshness gates and blocks strict browser on deterministic audit failure | Formation only. Pin in a separate metadata boundary, then run every release stage from zero; public push/publication remains unauthorized |
...
| Public demos | Seven executable base scenarios, nine Genesis stages and two pack showcases exist | Exact rc26 passed its first/only 102-cell public matrix with 0 skips/retries, including demo, pack, Genesis, accessibility, compatibility, failure and source-lifecycle cells; rc27 stopped in Python before browser, rc28/rc29 were rejected pre-validation, rc30 was rejected before its complete matrix and rc31 stopped on operational-pass freshness before browser | Gallery, source/failure/compatibility/accessibility worlds and pack Chronoscope are concrete | Prior browser/capture proof remains exact-subject historical evidence only; rc32 must repeat every stage |
| Visual system | Light/dark themes, three densities, semantic tokens, licensed asset manifest, WebGL and 2D fallback render | Exact rc26 passed 102 browser cells and its first productive four-profile capture, manifest `6681e1f751ecd157854a4c3d78360a79f981100a4eda97ec377189ea9566614f`; rc27 never reached capture, rc28/rc29 were rejected pre-validation, rc30 stopped before its matrix on RT-164 and rc31 stopped before browser on RT-165 | Native/fallback/profile behavior was captured without granting release authority; transition frames still require settling and VoiceOver remains human | Rc24 and rc26 visual evidence is historical and cannot be reused. Rc32 requires a fresh productive capture, certification and human conceptual/privacy/VoiceOver review |
...
for a separately certified exact rc32-or-later subject. Public closure never
...
| RT-150 / RT-162 / RT-163 / RT-164 / RT-165 / RT-166 | rc26 became `candidate` only after exact validation, then passed productive capture and all six registered commands; the public-evidence scanner still rejected the transaction before attestation. Rc27 then stopped in its first `-W error` Python validation with 46 lifecycle failures before browser or candidate formation. Rc28 source `31cad3bc...` implemented the lifecycle corrections but was rejected for stale portable guides. Rc29 source `905e3772...` fixed those guides but was rejected for a stale portable skill and private-lineage fixture labels. Rc30 source `bc44255b...` preserved the fixes, then real-data visual QA exposed identical labels on distinct quadrant-family controls before its complete matrix. Rc31 source `6fa9b907...` preserved RT-164 but failed its first complete validation on stale, non-fixed-point operational-pass output after 1,740 Python and 517 frontend passes; guide/CI topology did not fully enforce the canonical required-gate order | Preserve rc26 as immutable failed-certification evidence, rc27/rc31 as immutable failed-validation evidence and rc28/rc29/rc30 as immutable pre-validation rejections. Rc32 must exclude the generated dashboard from its own recent-page projection, prove one-write idempotence, align local/CI warning and freshness commands, block strict browser on deterministic audit/test failure, regenerate once and then use wholly new validation/candidate/capture/certification boundaries before fresh private preflight or any promotion boundary |
...
| Runner identity was only a human version label | Runner `1.3.0+payload.<sha256>` embeds a digest of byte/mode entries for its entrypoint, bootstrap helpers (`_common.py` and `_git_subject.py`), portable toolchain probe, `wiki_core/**/*.py` and runtime schemas; `toolchain_sha256` binds the complete portable closure. The closure uses plan v4, mutation state v2, runner state v4, runner report v3, adoption receipt v4, acceptance attempt v2 and boundary operations v2. | Same base version with altered entrypoint, either bootstrap helper, probe or a core dependency is rejected before consumer mutation; byte-equal identity is path-independent |
...
before its complete matrix on RT-164. All new certification must begin from a
separately pinned and fully validated rc32-or-later source. Public
...
`6fa9b907d5dfc748e94d182ac3704b226142552e` is pinned only for validation;
complete validation and every later authority stage must start from zero.
Public push and publication remain
unauthorized.

#### Rc22 failed capture, rc23/rc25/rc27/rc31 failed validation, rc24/rc26 failed certification, rc28/rc29/rc30 rejected and rc32 formation — 2026-07-14
...
| Rc32 source formation | Active package `unreleased@6c8fce74`; no rc32 source pin | Stale-target, missing-target, JSON, custom-output and relative-root fixed-point regressions pass; 198 focused operational/workflow/package/lane tests pass with `-W error`; one regeneration is immediately check-clean at dashboard SHA-256 `b632bf8e...` | Prospective only. Form and pin in separate commits, then execute every validation and release stage from zero; no public push/publication authority |

Rc27 and rc31 are frozen as historical failed-validation evidence, rc28/rc29
are frozen as pre-validation rejections, rc30 is frozen as a pre-validation
rejection and rc32 is prospective and unpinned. The
...
rc32-or-later authority exists. The rehearsal must use a fresh clone and branch
...
import it. Form rc32 with the one-write operational-pass fixed point, public
synthetic stale/missing-target regressions, exact local/CI warning policy and
strict-browser dependency on deterministic audit success. Pin rc32 only in a
separate metadata boundary, then run one complete exact validation, a separate
candidate boundary, one new productive capture, one certification and
independent capsule verification from zero. Keep rc32 local and
...
rc30 formation. No public push is authorized by this plan update.
