...
- [scripts/wiki_archive.py](../../scripts/wiki_archive.py): physically archives resolved proposals (superseded/rejected) and their events into ingestao/arquivo/ via git mv, transitioning the gate to archived, adding stale_exempt and rewriting inbound links/refs.
...
- Doc-code gate: every tracked `wiki_*.py` CLI must appear in [referencia-comandos.md](wiki/referencia-comandos.md) and vice versa (errors both ways).
...
- New meta-wiki page [memorias/sistema/wiki/custos-operacao.md](wiki/custos-operacao.md): where the cost actually goes (agent session and human time dominate; ingestion is pocket change) and the levers (session discipline, budget alert, Batches, model by profile). Registered in the meta-wiki index.
...
- Living wiki initialized from the reusable kit: deterministic core in
  ...
  [docs/referencias/templates/wiki/](../../docs/referencias/templates/wiki/).
- Example context created ([exemplo/index.md](../exemplo/index.md)); contexts
...
- `language: en` set in [wiki.config.yaml](../../wiki.config.yaml); the generated
  cockpit ([operacao.md](../operacao.md)) is now rendered in English via the
...
- All authored content translated to English: [AGENTS.md](../../AGENTS.md), every
  page under [memorias/](..) (methodology pages, meta-wiki, perceptive layer,
