---
page_id: sistema-wiki-operacao-diaria
...
materialized in [memorias/operacao.md](../../operacao.md). The human gate is the
GitHub PR, configured in [wiki.config.yaml](../../../wiki.config.yaml) (`gate:
github_pr`) and described in [aprovacoes-git.md](../aprovacoes-git.md). The process of
incorporating sources lives in [processo-ingestao.md](../processo-ingestao.md), and the
detailed automatic gates are in [gates-e-auditoria.md](gates-e-auditoria.md).
...
[contrato-wiki-operacional.md](../contrato-wiki-operacional.md). The golden rule:
...
The first action of the day is to open [memorias/operacao.md](../../operacao.md). It is the
...
content of memorias/ at HEAD; recompile before trusting it.
...
- Owner actions: table derived from the memory's action pages, with the state read from the
  body of each action (`Estado:`). It is the active work list.
...
- Context vitality: for each context hub (memorias/<ctx>/index.md
  of type `context_hub`), it compares `updated_at + stale_after_days` with today's
  date and marks `fresca` or `stale`. `stale` contexts are priority candidates
...
is in [processo-ingestao.md](../processo-ingestao.md); the PR approval cycle
is in [aprovacoes-git.md](../aprovacoes-git.md).

```sh
git switch -c wiki/<tema>
```
...
involves proposals in [memorias/sistema/ingestao/](../ingestao/):
...
[aprovacoes-git.md](../aprovacoes-git.md) and the approval parameters in
[wiki.config.yaml](../../../wiki.config.yaml) (`require_operation_update`,
`require_log_update`).

## 5. Recompile the cockpit

At the end of the round (and whenever memorias/, decisions or actions change), recompile
...
```sh
python3 scripts/wiki_operation_compile.py --check   # cockpit em dia?
git switch -c wiki/<tema>                            # branch de proposta
# ... editar memorias/, decisoes, acoes ...
python3 scripts/wiki_audit.py --check                # gate 1: estrutura/links/segredos
python3 scripts/wiki_pr_summary.py                   # gate 2: resumo + checklist do PR
git diff --check                                     # gate 3: higiene de diff
python3 scripts/wiki_operation_compile.py --write    # recompila o cockpit
# ... abrir/atualizar o PR (gate humano) ...
```

## Related

- Generated cockpit: [memorias/operacao.md](../../operacao.md)
- Root MOC: [memorias/index.md](../../index.md)
- Gates and audit: [gates-e-auditoria.md](gates-e-auditoria.md)
- Approvals by Git/PR: [aprovacoes-git.md](../aprovacoes-git.md)
