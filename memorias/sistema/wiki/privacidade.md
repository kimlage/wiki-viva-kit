---
page_id: sistema-wiki-privacidade
...
[gates and audit](gates-e-auditoria.md) and [Git approvals](../aprovacoes-git.md).
...
walks the Markdown files under [memorias/](../../), reads the `visibility` from the frontmatter (falling back to the
...
category `entity`, severity `baixo` — purely informational, to label personal
...
- **Generic assignment with entropy filter**: names like `api_key`, `secret`,
  `token`, `senha`, `password`, `client_secret`, `refresh_token` followed by `:` or `=`.
  ...
  **Shannon entropy above 3.0 bits/character** (`_looks_random`). Thus,
  `senha: minha frase comum` does not beep, but `senha: 9f3aK7xQ2mZ…` beeps. The calculation is
...
- **`public_candidate`**: the function `audit_public_candidates` scans **all** the pages
  ...
  separately — see [Git approvals](../aprovacoes-git.md) and the
  [operational wiki contract](../contrato-wiki-operacional.md).
...
- Everything the auditor reports already comes redacted; the pipeline that triggers these checks is in
  [ingestion process](../processo-ingestao.md) and the method coverage in
  [methodology coverage v5](../cobertura-metodologia-v5.md).
