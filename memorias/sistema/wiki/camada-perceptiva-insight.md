---
page_id: sistema-wiki-camada-perceptiva-insight
...
[perception_policy](#perception_policy-a-marca-da-camada), and does not depend on
isolated human discipline: the gate by PR and the auditing guarantee that nothing
crosses the boundary by accident. See
[contrato-wiki-operacional.md](../contrato-wiki-operacional.md) and
...
template in [docs/referencias/templates/wiki](../../../docs/referencias/templates/wiki/journal.md).
...
is [journal.md](../../../docs/referencias/templates/wiki/journal.md). Three structural
...
- The `## Promover? (consentimento)` section — the controlled exit point: the author
...
[mapa-relacoes.md](../../../docs/referencias/templates/wiki/mapa-relacoes.md). It
...
The templates already reserve the `## Alternativa textual acessivel` section for this.
...
The real hub of the layer is [index.md](../percepcao/index.md), in
[memorias/sistema/percepcao](../percepcao/index.md), which lists the living perceptive
pages and reaffirms: nothing there is a source of truth and every promotion requires
consent + PR.

## The Information -> Insight cycle

Manual promotion (the journal's `## Promover?` section) covers the case where the author
...
[processo-ingestao.md](../processo-ingestao.md): the Python code is
...
- **Score events** — read from the karma event trail and filtered by the
  ...
  karma mechanics are in [gamificacao-karma.md](gamificacao-karma.md).
...
- **Memory pages** — a scan of [memorias](../../index.md) for pages
...
2. **Skeleton proposal** (Markdown) — a `page_type: insight` page with
   `status: candidato`, `status_epistemologico: candidato` and `gate: github_pr`,
   with `## Leitura`, `## Evidencias reunidas`, `## Incerteza` and
   `## Possivel acao` sections. The interpretive fields come with "To be filled by the
   agent"; only the counted evidence comes pre-filled.
...
3. **The human** reviews the proposal and promotes it (or not) to memory via PR, under the
   auditing described in [gates-e-auditoria.md](gates-e-auditoria.md) and the flow
   of [aprovacoes-git.md](../aprovacoes-git.md).
...
[operacao.md](../../operacao.md), and the living hub of the layer is
[index.md](../percepcao/index.md). The cycle does not replace the manual perception of the
...
- Hub of the perceptive layer: [index.md](../percepcao/index.md).
- Journal template: [journal.md](../../../docs/referencias/templates/wiki/journal.md).
- Relations map template: [mapa-relacoes.md](../../../docs/referencias/templates/wiki/mapa-relacoes.md).
- Insight job (code): [job.py](../../../wiki_core/insight/job.py) and CLI [wiki_insight_job.py](../../../scripts/wiki_insight_job.py).
- Same architecture in the ingestion (LLM pass): [processo-ingestao.md](../processo-ingestao.md).
- Gates, auditing and secret detection: [gates-e-auditoria.md](gates-e-auditoria.md).
- Karma and scoring: [gamificacao-karma.md](gamificacao-karma.md).
- Page and promotion contract: [contrato-wiki-operacional.md](../contrato-wiki-operacional.md).
- PR approval flow: [aprovacoes-git.md](../aprovacoes-git.md).
- Method coverage matrix: [cobertura-metodologia-v5.md](../cobertura-metodologia-v5.md).
