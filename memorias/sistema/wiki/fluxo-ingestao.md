---
page_id: sistema-wiki-fluxo-ingestao
...
[ingestion process](../processo-ingestao.md); the blocking criteria and the
gate mechanics are in [gates and auditing](gates-e-auditoria.md).
...
```
manifesto -> texto + chunks -> indice -> pre-scan -> pacote de contexto LLM
          -> passagem LLM (agente, no cache) -> evento normalizado (quadrantes)
          -> consolidacao -> PR
```
...
is in [gates and auditing](gates-e-auditoria.md).
...
[git approvals](../aprovacoes-git.md) and [gates and auditing](gates-e-auditoria.md).
...
created -> compiling -> ready_for_review -> needs_human_gate
        -> approved -> published
(qualquer um pode ir para superseded / rejected / archived conforme as arestas)
```
...
[v5 methodology coverage](../cobertura-metodologia-v5.md)). It is the only write
...
- [Ingestion process](../processo-ingestao.md) - the high-level operational procedure
  corresponding to this flow.
- [Gates and auditing](gates-e-auditoria.md) - blocking (secret) vs
  informing (PII) criteria and the gate-by-PR mechanics.
- [Operational wiki contract](../contrato-wiki-operacional.md) - general rules of the
  wiki, including links and sensitive data.
- [Git approvals](../aprovacoes-git.md) - branch and PR convention.
- [v5 methodology coverage](../cobertura-metodologia-v5.md) - where karma,
...
- Operational cockpit: [operacao.md](../../operacao.md). Root MOC:
  [index.md](../../index.md).
