---
page_id: sistema-wiki-gamificacao-karma
...
- Append-only: the "Score Keeper" only writes events, never edits or deletes the
  history. Each event is a JSON line in
  [score-events.jsonl](../../../data/derived/wiki/score-events.jsonl).
...
[Approvals via Git](../aprovacoes-git.md) and [Gates and auditing](gates-e-auditoria.md);
for the flow that triggers the events, see [Ingestion process](../processo-ingestao.md).
...
[dashboard-vitalidade.md](../../../docs/referencias/templates/wiki/dashboard-vitalidade.md).
...
The cockpit [Operacao](../../operacao.md) has a "Karma and vitality
...
python3 scripts/wiki_score.py --add --event criar_insight_aceito \
    --actor owner --context exemplo --quality 1.0 --impact 3 --rare
...
- Operational cockpit: [Operacao](../../operacao.md)
- Ingestion process (origin of the events): [processo-ingestao.md](../processo-ingestao.md)
- Gates and auditing: [gates-e-auditoria.md](gates-e-auditoria.md)
- Approvals via Git: [aprovacoes-git.md](../aprovacoes-git.md)
