---
page_id: sistema-wiki-referencia-comandos
...
This page catalogs the deterministic CLIs of the living wiki system. They all live in [scripts/](../../../scripts/) with the `wiki_` prefix, are pure Python (with no external dependency beyond PyYAML), call no language model and read the repo profile from [wiki.config.yaml](../../../wiki.config.yaml) via [wiki_core/config.py](../../../wiki_core/config.py). The deep reading (LLM) is always delegated to the agent that runs the repo, as per [processo-ingestao.md](../processo-ingestao.md). The gates and the audit are detailed on the sister page [gates-e-auditoria.md](gates-e-auditoria.md), and the PR approval cycle in [aprovacoes-git.md](../aprovacoes-git.md).
...
| [wiki_archive.py](../../../scripts/wiki_archive.py) | Archives resolved proposals (and events) into ingestao/arquivo/ | Take immutable history out of the flat, re-audited dir |
...
| [wiki_operation_compile.py](../../../scripts/wiki_operation_compile.py) | Compiles the daily cockpit | (Re)generate [memorias/operacao.md](../../operacao.md) |
...
- `--context` (required): target context (e.g.: `sistema`).
...
Generates the Markdown file of the private proposal (in [memorias/sistema/ingestao/](../../../memorias/sistema/ingestao/README.md)) with frontmatter, quadrants, privacy risks and checklist. It classifies the source (raw/memory/reference/artifact), does a pre-triage (secrets block; PII is only reported) and, on writing, applies a rebase to supersede pending proposals of the same logical target. The context -> pages/entities map comes from `wiki.targets.yaml` (per-repo profile), keeping the script generic.
...
```sh
python3 scripts/wiki_new_ingest.py --source data/raw/exemplo.pdf --context sistema
python3 scripts/wiki_new_ingest.py --source X.md --context sistema --dry-run
```
...
```sh
python3 scripts/wiki_extract_source_manifest.py --source data/raw/exemplo.pdf --context sistema --dry-run
```
...
```sh
python3 scripts/wiki_extract_text.py --source data/raw/exemplo.pdf --context sistema --dry-run
python3 scripts/wiki_extract_text.py --source data/raw/exemplo.pdf --context sistema
```
...
Calls no model at all. It gathers/selects chunks (by source or by sanitized FTS search), assembles a context PACKAGE (prompt + schema + chunk text), records in the cache the RESULT that the agent produced and serves as a gate (`--check`). The `--emit-request` writes the `-llm-context-request.json` that [wiki_audit.py](../../../scripts/wiki_audit.py) watches. Gate details in [gates-e-auditoria.md](gates-e-auditoria.md).
...
python3 scripts/wiki_llm_context_pass.py --query "decisoes pendentes" --context sistema
python3 scripts/wiki_llm_context_pass.py --record-result resultado.json --context sistema
...
- `--context` (default `sistema`).

```sh
python3 scripts/wiki_cache_inspect.py --summary
python3 scripts/wiki_cache_inspect.py --source data/raw/exemplo.pdf
```
...
Lists proposals, applies valid state transitions and rebases/supersedes pending proposals of the same logical target. By default it operates in [memorias/sistema/ingestao/](../../../memorias/sistema/ingestao/README.md) (proposals live flat). It requires exactly one action: `--list`, `--transition` or `--rebase`. States and transition machine detailed in [gates-e-auditoria.md](gates-e-auditoria.md).

- `--dir`: proposals directory (default [memorias/sistema/ingestao/](../ingestao/)).
...
python3 scripts/wiki_gate.py --transition memorias/sistema/ingestao/2026-06-09-sistema-exemplo.md --to approved --reason "revisado"
python3 scripts/wiki_gate.py --rebase --rebase-key sistema-exemplo
```

### [wiki_score.py](../../../scripts/wiki_score.py) - karma and vitality

Gamification layer: records and aggregates append-only scoring events in [data/derived/wiki/score-events.jsonl](../../../data/derived/wiki/), without a toxic global ranking. It acts as a Score Keeper (never edits history). It requires one action: `--add`, `--summary` or `--dashboard`.
...
- `--events-path`: path of the JSONL (default [data/derived/wiki/score-events.jsonl](../../../data/derived/wiki/score-events.jsonl)).

```sh
python3 scripts/wiki_score.py --add --event ingestar_fonte_valida --actor owner --context sistema
python3 scripts/wiki_score.py --add --event criar_insight_aceito --actor owner --context exemplo --quality 1.0 --impact 3 --rare
...
python3 scripts/wiki_insight_job.py --theme "conciliacao" --context exemplo --dry-run
```

### [wiki_operation_compile.py](../../../scripts/wiki_operation_compile.py) - daily cockpit

Compiles the operational cockpit [memorias/operacao.md](../../operacao.md) from real sources (config, decisions, actions, vitality of the context hubs, karma and Git state), never from hardcoded content. The `--check` compares only the DETERMINISTIC view (ignores date/commit/karma) with a recompile at HEAD, so that CI can require the cockpit to be up to date.

- (no flags): prints the cockpit to stdout.
- `--write`: writes to [memorias/operacao.md](../../operacao.md) and records a score-event idempotent per day.
...
See also the system page [cobertura-metodologia-v5.md](../cobertura-metodologia-v5.md).
...
4. Gate: [wiki_gate.py](../../../scripts/wiki_gate.py) to transition/supersede; see [aprovacoes-git.md](../aprovacoes-git.md).
...
For each agent's protocol, see [AGENTS.md](../../../AGENTS.md). Back to the root MOC in [memorias/index.md](../../index.md) and to the cockpit in [memorias/operacao.md](../../operacao.md).
