---
page_id: sistema-processo-ingestao
...
3. Define the target context among those declared in [wiki.config.yaml](../../wiki.config.yaml)
   (`contexts`); `sistema` is always valid.
...
6. Record a normalized event in [memorias/sistema/ingestao/eventos/](ingestao/eventos/)
...
8. If the decision is not trivial, generate a proposal in
   [memorias/sistema/ingestao/YYYY-MM-DD-<tema>.md](ingestao/).
...
11. Turn every cited local path into a clickable Markdown link. The label may be
   ...
   original source.
12. Update related pages and [memorias/sistema/log.md](log.md).
...
- Accepted examples: [memorias/exemplo/index.md](../exemplo/index.md),
...
When a quadrant does not appear in the source, fill in the `Ausencia/limite`
