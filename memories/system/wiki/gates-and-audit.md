...
[wiki_operation_compile.py](../../../scripts/wiki_operation_compile.py) recompiles the cockpit ([memories/operations.md](../../operations.md)) from the content of [memories/](../../) and, with `--check`, fails if the result diverges from what is versioned. The comparison uses `stable_cockpit_view`: it covers the ENTIRE deterministic body (stable frontmatter, state now, decisions, actions, queue, alerts, links) — not just three sections — and removes the volatile parts (date, git state, karma/score) so that "up to date" means "the deterministic content matches a recompile at HEAD". Commit hash equality is not required by design (the recompile commit cannot contain its own hash, and merges always differ); the gate is the content, not the hash.
